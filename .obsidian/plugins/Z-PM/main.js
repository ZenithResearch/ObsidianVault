...
var ProjectMan = class extends import_obsidian.Plugin {
  constructor() {
    super(...arguments);
    this.projects = [];
    ...
    this.addCommand({
      ...
      editorCallback: (editor, view) => {
        ...
        if (adapter instanceof import_obsidian.FileSystemAdapter) {
          ...
          let git = esm_default(`${basePath}/${view.file.parent.path}`);
          if (this.currentProject) {
            git = esm_default(`${basePath}/7_Projects/${this.currentProject.name}`);
          ...
          if (!frontMatter) {
            ...
            GenericSuggester.Suggest(this.app, this.projects.map((p, i, a) => p.name), this.projects.map((p, i, a) => p.name)).then((res) => {
              ...
              git = esm_default(`${basePath}/7_Projects/${res}`);
            });
          }
          if (frontMatter) {
            let cwd = process.cwd();
    ...
    this.addCommand({
      ...
      editorCallback: (editor, view) => {
        ...
        if (adapter instanceof import_obsidian.FileSystemAdapter) {
          let basePath = adapter.getBasePath();
          if (this.currentProject) {
            let git = esm_default(`${basePath}/${view.file.parent.path}/wiki`);
            git.add(".").then((addRes) => {
          ...
          try {
            ...
            GenericSuggester.Suggest(this.app, this.projects.map((p, i, a) => p.name), this.projects.map((p, i, a) => p.name)).then((res) => {
              let git = esm_default(`${basePath}/7_Projects/${res}/wiki`);
              gitAddCommit(git, "Wiki Update (Obsidian)");
            });
          }
        }
      ...
      editorCallback: async (editor, view) => {
        ...
        if (project) {
          const ghArgs = {
            ...
            assignees: await ((_a2 = this.app.plugins.plugins.quickadd) == null ? void 0 : _a2.api.inputPrompt("Assignees", "Enter Assignees (Comma-Separated)")),
            title: await ((_b = this.app.plugins.plugins.quickadd) == null ? void 0 : _b.api.inputPrompt("Title", "Title of issue")),
            ...
            tags: await ((_c = this.app.plugins.plugins.quickadd) == null ? void 0 : _c.api.inputPrompt("Tags", "Enter Labels (Comma-Separated)"))
          };
          ...
          let result = (0, import_child_process2.exec)(command, (err, stdout, stderr) => {
            var _a3;
            console.log(`OUT > ${stdout}`);
            console.log(`ERR > ${stderr}`);
            if (stdout && !err) {
              (_a3 = this.app.plugins.plugins.quickadd) == null ? void 0 : _a3.api.executeChoice("Create Issue Note", {
                ...
                var _a4;
                (_a4 = this.app.plugins.plugins.quickadd) == null ? void 0 : _a4.api.executeChoice("Capture Issue");
              });
            }
          });
        } else {
      ...
      editorCallback: (editor, view) => {
        ...
        if (adapter instanceof import_obsidian.FileSystemAdapter) {
          ...
          this.projects.forEach((v) => {
            let git;
            try {
              git = esm_default(`${basePath}/7_Projects/${v}/wiki`);
            } catch (e) {
              git = esm_default(`${basePath}/${view.file.parent.path}/wiki`);
            }
            gitAddCommit(git, "Wiki Update (Obsidian)");
          });
        }
...
async function gitAddCommit(git, message) {
  if (git) {
    git.add(".", (addRes) => {
      console.log(`Added: ${addRes}`);
...
function getObsidianFrontmatter(doc) {
...
var ZPMSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    this.plugin.projects.forEach((p) => {
      new import_obsidian.Setting(containerEl).setName("Name").setDesc(p.name).addText(
        (text) => text.setPlaceholder("Project Name").setValue(p.name).setDisabled(true)
      ).addText(
        (text) => text.setPlaceholder("Project Wiki").setValue(p.wiki).setDisabled(true)
      ).addText(
        (text) => text.setPlaceholder("Project Repo").setValue(p.repo ? p.repo : "").onChange(async (value) => {
          console.log("repo: " + value);
          p.repo = value;
          await this.plugin.saveProjects();
        })
      ).addText(
        (text) => text.setPlaceholder("Project Board").setValue(p.projectBoard ? p.projectBoard : "").onChange(async (value) => {
...
var GenericSuggester = class extends import_obsidian.FuzzySuggestModal {
